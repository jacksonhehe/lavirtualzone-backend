...
router.post('/me/train', auth, async (req, res) => {
  try {
    ...
    await player.save();
    // Registrar la transacción con tipo en minúsculas para mantener la consistencia
    await Transaction.recordTransaction(req.user.id, club._id, 'entrenamiento', player.name, cost, player._id);
